...
\subsection{Isoscalar Correction}
	...
	\end{equation}	
	This isoscalar correction can be determine by applying correction function $f_{iso}$ to equate the two and solving that combination for the function.
	\begin{equation}
	...
	\end{equation}
	The isoscalar correction for a nuclear target depends on A,Z, and the F$_2$ ratio. There exist no data on the free neutron, so the $F^n_2/F^p_n$ is a model dependent extraction. The  F$_2$ ratio has been extracted from a ratio of tritium to helium from the MARATHON experiment and  hydrogen to deuterium ratios from previous experiments from SLAC and the NMC. I have used the $F^n_2/F^p_n$ from MARATHON to calculate my iscscalar correction. I have estimated the error of this model dependent correction by comparing the isoscalar correction from the MARATHON extraction with the isoscalar correction using three other $F^n_2/F^p_n$ models Kulagin, S. A. and Petti, R. \cite{kpmodel}, J. Arrington \cite{JA_FR}, and NMC \cite{NMC_ratio}. The relative variance of this calculation ranges from $\pm$ 0.1\% to $\pm$ 3.8\% for tritium EMC ratio. These 4 models and the error band from this study is shown in figure \ref{isofuncs}.

	\begin{figure}[t]
		\includegraphics[width=15cm]{ISOfacs.pdf}
...
\paragraph{}The EMC Effect isoscalar corrected ratios for helium-3 and tritium from my inclusive DIS cross section extraction are displayed in this section. The data for each of these targets were taken with the same spectrometer in rotating fashion to help eliminate some of the systemic uncertainties.
